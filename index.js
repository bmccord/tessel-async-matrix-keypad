...
Keypad.prototype = {
    ...
    isPressed: function (key, col, callback) {
        var self = this;
        var matched = false;

        if (!callback && typeof(col) === "function") {
            callback = col;
            col = null;
        }
        ...
        async.series([
            function (seriesCallBack) {
                self.getPressed(seriesCallBack);
            },
            function (seriesCallBack) {
                if (!isNaN(key)) {
                    matched = self.keys[self.layout[key][col]];
                } else {
                    ...
                    keys.forEach(function (key) {
                        if (self.keys[key]) matches++;
                    }, self);

                    matched = matches === keys.length;
                }
                seriesCallBack();
            }
